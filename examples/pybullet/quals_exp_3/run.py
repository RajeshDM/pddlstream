...
def pddlstream_from_problem(unity_agent, problem, teleport=False):
    ...
    floor = 'floor'
    floor_points = [{'x':0,'y':0,'z':0}]*5
    #ic (floor_points)
    floor_pose = Pose(floor,value=floor_points,support=0.0)
    #ic (floor_pose.__dict__)
    #exit()
    ...
    if unity_agent == None :
        ...
        init = [
            ...
            ('Pose',floor,floor_pose),
            ('AtPose',floor,floor_pose),
            #('FloorObject',floor),
        ...
        for body,required_points in problem.unity_objects:
            ...
            init += [('Pose', body['uuid'], pose),
                     ...
                     ('Supports', body['uuid'],pose,floor,floor_pose)]
            #ic (pose.__dict__)
            trophy_new_points = []
            if body['id'] == 'trophy':
                #ic (pose.__dict__)
        ...
        #goal = ("AtPose",'trophy_1', trophy_new_pose)
        #goal = ("PickedUp", pickup_id_1)
        #ic (trophy_new_pose.__dict__)
        #init += [('Supports', 'trophy', trophy_new_pose,floor,floor_pose)]
